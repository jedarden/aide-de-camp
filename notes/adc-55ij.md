...
5. **No authorization context.** Pod deletion is hard-to-reverse and there was no
   ...
   intent) and notes the critical missing information (no pod name / namespace),
   instructing the executor not to guess.
...
than a task-profile intent, with explicit "CRITICAL MISSING INFORMATION" guidance:
do not guess the pod name; ask the user to specify which pod; require explicit
confirmation for any delete-all. Those steps cannot be satisfied here because no
pod name, namespace, or cluster was supplied and no interactive user is present to
supply them. The correct disposition for a real `kubectl delete pod` utterance is
...
`kubectl delete pod` test fixture (see [[adc-4eqs]], [[adc-6zfb]], [[adc-3h2c]],
[[adc-65lc]], and [[adc-2rh8]] for the most recent refusals; earlier occurrences
