...
5. **No authorization context.** Pod deletion is hard-to-reverse and there was no
   ...
   pod name was not provided and instructs the executor to identify/confirm the
   target before deletion — a step that cannot be satisfied here.
...
The bead description flags `kubectl delete pod` as an immediate-execution `action`
intent that requires a specific context — a pod name and namespace — to be
actionable, and notes that the pod name was omitted from the utterance. It
instructs the executor to list pods and have the user specify the target when it
is missing. None of those preconditions are satisfiable here: no pod name,
namespace, or cluster was supplied, and no interactive user is present to supply
them. The correct disposition for a real `kubectl delete pod` utterance is to
confirm the missing arguments (pod name, namespace, cluster) with the user first,
and to drive the change through declarative-config (or a GitOps-driven
rollout/restart) rather than a manual `kubectl delete`.
...
`kubectl delete pod` test fixture (see [[adc-4so2]], [[adc-5cnz]], [[adc-414x]],
