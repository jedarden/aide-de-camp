...
5. **No authorization context.** Pod deletion is hard-to-reverse and there was no
   ...
   pod name was not provided and classifies the request as requiring investigation
   (identifying the pod) before any action can be taken — a step that cannot be
   satisfied here.
...
actionable, and notes that the pod name and namespace were omitted from the
utterance. It explicitly states this fits a **task-profile** escalation because it
requires investigation (identifying the pod) before action can be taken, and its
implementation notes call for listing pods and asking the user which specific pod
to delete when the target is missing. None of those preconditions are satisfiable
here: no pod name, namespace, or cluster was supplied, and no interactive user is
present to supply them. The correct disposition for a real `kubectl delete pod`
utterance is to confirm the missing arguments (pod name, namespace, cluster) with
the user first, and to drive the change through declarative-config (or a
GitOps-driven rollout/restart) rather than a manual `kubectl delete`.
...
`kubectl delete pod` test fixture (see [[adc-1uwx]], [[adc-4so2]], [[adc-414x]],
[[adc-6zfb]], [[adc-1ogd]], [[adc-55ij]], [[adc-4eqs]], [[adc-3h2c]], [[adc-65lc]],
and [[adc-5jl]] for prior refusals; earlier occurrences were logged as
