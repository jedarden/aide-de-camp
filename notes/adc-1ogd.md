...
5. **No authorization context.** Pod deletion is hard-to-reverse and there was no
   ...
   `kubectl delete pod` as an action intent whose success depends on a target pod
   that "was not provided in the user utterance," and instructs the executor to
   ask the user to specify the pod name when the target is ambiguous — a step that
...
The bead description itself flags `kubectl delete pod` as an action intent and
notes the critical missing information: the specific pod name "was not provided
in the user utterance" and "this command requires a target resource name to
execute." The implementation steps (retrieve pod list → ask the user to specify
if ambiguous → execute) cannot be satisfied here because no pod name, namespace,
or cluster was supplied and no interactive user is present to supply them. The
...
drive the change through declarative-config rather than a manual
`kubectl delete`.
...
`kubectl delete pod` test fixture (see [[adc-55ij]], [[adc-4eqs]], [[adc-6zfb]],
[[adc-3h2c]], and [[adc-65lc]] for the most recent refusals; earlier occurrences
