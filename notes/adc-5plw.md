...
5. **No authorization context.** Pod deletion is hard-to-reverse and there was no
   ...
   before execution "to prevent accidental data loss or downtime" — a step that
...
**incomplete and requires a specific pod name (and potentially a namespace)** to
execute, with implementation notes stating the required action is to "identify
the target," "verify context," and — critically — "prompt the user to confirm the
target before execution to prevent accidental data loss or downtime." None of
those preconditions are satisfiable here: no pod name, namespace, or cluster was
...
`kubectl delete pod` test fixture (most recently [[adc-4i74]], and earlier
[[adc-1uwx]], [[adc-4so2]], [[adc-414x]], [[adc-6zfb]], [[adc-1ogd]],
[[adc-55ij]], [[adc-4eqs]], [[adc-3h2c]], [[adc-65lc]], [[adc-5jl]]). All were
declined on the same grounds: unscoped target, GitOps-only write policy,
read-only RBAC on most clusters, and no authorization/interactive confirmation. A
routing guard that refuses bare destructive `kubectl` writes — or that requires
