...
environment's safety posture forbids. This is the **fourth** identical occurrence of this
...
intent matches `kubectl delete pod` but the source utterance carries no operand. Four
recurrences strongly suggests the durable fix is **upstream**: the escalate
...
`src/escalate/handler.py:305-306` via `_execute_delete_pod`) — so the bare command would
...
- Escalate system implements `kubectl delete pod` as an escalate intent:
  ...
  - `src/escalate/handler.py:305-306,347` — dispatch + `_execute_delete_pod`
...
specify a target — same outcome as the three prior identical beads.
