...
The bead description flags `kubectl delete pod` as an immediate-execution
`action` intent — transient, single-turn command execution — that does not meet
the criteria for a `task-profile` escalation (durable, multi-step work). It is
explicitly out of scope for the Escalate Strand and is noted as incomplete
(missing the specific pod name) with no project context or success criteria. The
...
drive the change through declarative-config (or a GitOps-driven rollout/restart)
rather than a manual `kubectl delete`. None of those preconditions are
satisfiable here: no pod name, namespace, or cluster was supplied, and no
interactive user is present to supply them.
...
- Reserve `--force --grace-period=0` for a pod genuinely stuck in `Terminating`,
  after the target is confirmed.
...
`kubectl delete pod` test fixture (most recently [[adc-33sh]], and earlier
[[adc-5pen]], [[adc-5plw]], [[adc-4i74]], [[adc-1uwx]], [[adc-4so2]],
[[adc-414x]], [[adc-6zfb]], [[adc-1ogd]], [[adc-55ij]], [[adc-4eqs]],
[[adc-3h2c]], [[adc-65lc]], [[adc-5jl]]). All were declined on the same grounds:
unscoped target, GitOps-only write policy, read-only RBAC on most clusters, and
no authorization/interactive confirmation. A routing guard that refuses bare
