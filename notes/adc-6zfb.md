...
"Discovery — Run `kubectl get pods` to list available pods" and "Confirmation —
If multiple pods are present or the target is ambiguous, pause and ask the user
to specify which pod to delete." Those guardrails cannot be satisfied here
because no pod name, namespace, or cluster was supplied and no interactive user
is present to supply them. The correct disposition for a real
...
declarative-config rather than a manual `kubectl delete`.
...
`kubectl delete pod` test fixture (see [[adc-3h2c]], [[adc-65lc]], [[adc-2rh8]],
[[adc-3uum]], [[adc-3017]], and [[adc-5bwk]] for the most recent refusals;
