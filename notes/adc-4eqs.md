...
5. **No authorization context.** Pod deletion is hard-to-reverse and there was no
   ...
   `kubectl delete pod` as a transient action intent (not a durable task-profile
   intent) and routes it to an Execute Strand — but even an Execute Strand cannot
   invent a missing pod name, namespace, or cluster.
...
The bead description itself flags `kubectl delete pod` as an action intent rather
than a task-profile intent, and states the correct handling is an Execute Strand
that parses the command, identifies the target pod (via context or user prompt),
executes, and returns the result. Those steps cannot be satisfied here because
no pod name, namespace, or cluster was supplied and no interactive user is
...
the user first, and to drive the change through declarative-config rather than a
...
`kubectl delete pod` test fixture (see [[adc-6zfb]], [[adc-3h2c]], [[adc-65lc]],
[[adc-2rh8]], and [[adc-3uum]] for the most recent refusals; earlier occurrences
