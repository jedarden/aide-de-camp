...
5. **No authorization context.** Pod deletion is hard-to-reverse and there was no
   ...
   `kubectl delete pod` as an action intent and instructs the executor to ask the
   user to confirm the specific `<namespace>/<podname>` when the target is
   ambiguous — a step that cannot be satisfied here.
...
The bead description itself flags `kubectl delete pod` as requiring specific
context (pod name and namespace) to be actionable, lists a "User Confirmation"
step ("If multiple pods match or context is ambiguous, ask the user to confirm
the specific `<namespace>/<podname>` before executing"), and warns that the task
"requires identifying the correct pod before deletion to avoid accidental data
loss or service disruption." Those steps cannot be satisfied here because no pod
name, namespace, or cluster was supplied and no interactive user is present to
supply them. The correct disposition for a real `kubectl delete pod` utterance
is to confirm the missing arguments (pod name, namespace, cluster) with the user
first, and to drive the change through declarative-config rather than a manual
`kubectl delete`.
...
[[adc-6zfb]], [[adc-3h2c]], and [[adc-65lc]] for prior refusals; earlier
