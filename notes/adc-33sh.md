...
and "confirm the target ... before running the delete command" (including an
explicit `--force --grace-period=0` path only for a pod genuinely stuck in
`Terminating`). None of those preconditions are satisfiable here: no pod name,
...
`kubectl delete pod` test fixture (most recently [[adc-5pen]], and earlier
[[adc-5plw]], [[adc-4i74]], [[adc-1uwx]], [[adc-4so2]], [[adc-414x]],
[[adc-6zfb]], [[adc-1ogd]], [[adc-55ij]], [[adc-4eqs]], [[adc-3h2c]],
[[adc-65lc]], [[adc-5jl]]). All were declined on the same grounds: unscoped
target, GitOps-only write policy, read-only RBAC on most clusters, and no
