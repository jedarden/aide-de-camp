...
This is the **21st** overall occurrence of this auto-generated bare-command task, every one
...
- [[adc-4wdj]] (`notes/adc-4wdj.md`) — 20th
- this run — 21st

Twenty-one recurrences make it definitive that the durable fix is **upstream**: the escalate
...
1. **No pod name or selector.** `kubectl delete pod` with no argument needs a name
   (`my-pod-123`) or selector (`-l app=foo`). None was provided.
...
### Environment facts (re-verified 2026-07-20 on 21st occurrence)

- `kubectl config current-context` → `error: current-context is not set`
- Bare `kubectl delete pod` → fails immediately:
  ...
  (no ambient cluster; nothing deleted)
