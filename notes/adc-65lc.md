...
synchronous shell `action` that has been mis-routed into the escalate strategy
(designed for `intent_type: task-profile`, i.e. async, multi-step durable work).
Its own "Guidance" recommends routing to an Action Strand for "immediate shell
execution" — but that recommendation is unsafe to act on here because it ignores
two hard constraints: the GitOps-only write policy and the read-only RBAC on most
clusters, plus the absence of any interactive user to supply the missing pod
identifier. The correct disposition for a real `kubectl delete pod` utterance is
to confirm the missing arguments (pod name, namespace, cluster) with the user
first, and to drive the change through declarative-config rather than a manual
`kubectl delete` — not to escalate it into durable async work, and not to fire it
blind into a shell.
...
`kubectl delete pod` test fixture (see [[adc-2rh8]], [[adc-3uum]], [[adc-3017]],
[[adc-5bwk]], and [[adc-16w5]] for the most recent refusals; earlier occurrences
