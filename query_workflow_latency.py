#!/usr/bin/env python3
"""
Query workflow latency directly from Argo cluster for pbx-web and whisper-stt

This script queries the iad-ci cluster for workflow runs matching specific
templates and extracts latency metrics over a 30-day window.
"""
...
def query_workflows_from_cluster(template_name: str, start_date: str, end_date: str,
                                  service_name: str) -> LatencyAnalyzer:
    """Query workflows from iad-ci cluster for specific workflow template."""
    print(f"\n  Querying {service_name} workflows from cluster")
    print(f"  Workflow template: {template_name}")

    analyzer = LatencyAnalyzer(start_date, end_date, service_name)

    try:
        # Query kubectl for workflows
        cmd = [
...
def main():
    ...
    print("="*70)
    print("Querying Latency Metrics from Argo Cluster")
    print("="*70)
    ...
    results = {
        "query_metadata": {
            ...
            "data_source": "argo-cluster-iad-ci"
        },
        "services": {}
    }
    ...
    pbx_analyzer = query_workflows_from_cluster(
        "pbx-web-build",
        start_date,
        end_date,
        "pbx-web"
    ...
    whisper_analyzer = query_workflows_from_cluster(
        "whisper-stt-build",
        start_date,
        end_date,
        "whisper-stt"
