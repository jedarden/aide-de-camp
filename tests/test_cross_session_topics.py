...
@pytest.fixture
async def seed_session(test_db_store) -> str:
    """Create a seed session for warming topics."""
    return await test_db_store.create_session()


@pytest.fixture
async def fresh_session(test_db_store) -> str:
...
@pytest.mark.asyncio
async def test_project_topic_created_as_cross_session(test_db_store, seed_session: str) -> None:
    ...
    topics = await test_db_store.get_active_topics(seed_session)
    assert len(topics) == 1
    topic = topics[0]
...
@pytest.mark.asyncio
async def test_cross_session_topic_reused_across_sessions(test_db_store, seed_session: str, fresh_session: str) -> None:
    """Test that cross-session topics are reused when starting a fresh session."""
    # Create cross-session topic in seed session
    seed_topic_id, created = await test_db_store.find_or_create_topic(
        label="pbx-web",
        session_id=seed_session,
        topic_type="project",
        project_slugs=["pbx-web"],
    ...
    fresh_topic_id, created = await test_db_store.find_or_create_topic(
        label="pbx-web",
    ...
    seed_topics = await test_db_store.get_active_topics(seed_session)
    fresh_topics = await test_db_store.get_active_topics(fresh_session)
...
@pytest.mark.asyncio
async def test_session_scoped_topic_not_reused_across_sessions(test_db_store, seed_session: str, fresh_session: str) -> None:
...
@pytest.mark.asyncio
async def test_get_previous_result_for_diff_finds_same_result_type(test_db_store, seed_session: str) -> None:
    ...
    previous = await test_db_store.get_previous_result_for_diff(topic_id, "status:whisper-stt")
...
@pytest.mark.asyncio
async def test_get_previous_result_for_diff_cross_session(test_db_store, seed_session: str, fresh_session: str) -> None:
    ...
    seed_result_id = await test_db_store.create_result(
        intent_id=None,
        topic_id=topic_id,
        session_id=seed_session,
        summary="Seed result",
...
@pytest.mark.asyncio
async def test_get_previous_result_for_diff_different_types(test_db_store, seed_session: str) -> None:
    ...
    status_result_id = await test_db_store.create_result(
        intent_id=None,
        topic_id=topic_id,
        session_id=seed_session,
    ...
    brainstorm_result_id = await test_db_store.create_result(
        intent_id=None,
        topic_id=topic_id,
        session_id=seed_session,
    ...
    status_previous = await test_db_store.get_previous_result_for_diff(topic_id, "status:pbx-web")
    ...
    brainstorm_previous = await test_db_store.get_previous_result_for_diff(topic_id, "brainstorm:pbx-web")
...
@pytest.mark.asyncio
async def test_step1_fresh_session_no_diff_strip(test_db_store, seed_session: str, fresh_session: str) -> None:
    ...
    result_id, has_diff = await test_db_store.create_result_with_diff(
        intent_id=None,
        topic_id=topic_id,
        session_id=fresh_session,
    ...
    result = await test_db_store.get_all_results()
...
@pytest.mark.asyncio
async def test_step6_in_session_shows_diff_strip(test_db_store, fresh_session: str) -> None:
    ...
    step1_result_id, has_diff = await test_db_store.create_result_with_diff(
        intent_id=None,
        topic_id=topic_id,
        session_id=fresh_session,
        summary="Step-1 result",
    ...
    step6_result_id, has_diff = await test_db_store.create_result_with_diff(
        intent_id=None,
        topic_id=topic_id,
        session_id=fresh_session,
...
@pytest.mark.asyncio
async def test_status_never_diffs_against_brainstorm(test_db_store, fresh_session: str) -> None:
    ...
    # Create brainstorm result
    brainstorm_result_id, _ = await test_db_store.create_result_with_diff(
        intent_id=None,
        topic_id=topic_id,
        session_id=fresh_session,
        summary="Brainstorm result",
    ...
    status_result_id, has_diff = await test_db_store.create_result_with_diff(
        intent_id=None,
        topic_id=topic_id,
        session_id=fresh_session,
        summary="Status result",
...
@pytest.mark.asyncio
async def test_cross_session_canvas_shows_only_current_session_results(test_db_store, seed_session: str, fresh_session: str) -> None:
    ...
    fresh_result_id = await test_db_store.create_result(
        intent_id=None,
        topic_id=topic_id,
        session_id=fresh_session,
    ...
    seed_results = await test_db_store.get_latest_results_by_type(seed_session)
    ...
    fresh_results = await test_db_store.get_latest_results_by_type(fresh_session)
...
@pytest.mark.asyncio
async def test_previous_result_id_set_cross_session(test_db_store, seed_session: str, fresh_session: str) -> None:
    ...
    # Create result in fresh session
    fresh_result_id, _ = await test_db_store.create_result_with_diff(
        intent_id=None,
        topic_id=topic_id,
        session_id=fresh_session,
        summary="Fresh result",
