...
class TestE2EHotPathDispatchTimings:
    ...
    @pytest.mark.asyncio
    async def test_hot_path_dispatch_captures_router_ms(self, test_db_store, mock_zai_client):
        ...
        timings = await test_db_store.get_dispatch_timings(routed_intent.intent_id)
    ...
    @pytest.mark.asyncio
    async def test_hot_path_partial_upsert_preserves_router_ms(self, test_db_store, mock_zai_client):
    ...
    @pytest.mark.asyncio
    async def test_all_server_stages_recorded_for_hot_path(self, test_db_store):
...
class TestE2EClientReportedTimings:
    ...
    @pytest.mark.asyncio
    async def test_client_stt_ms_persists_correctly(self, test_db_store):
    ...
    @pytest.mark.asyncio
    async def test_client_first_render_ms_persists_correctly(self, test_db_store):
        """Client-reported first_render_ms is stored and can be retrieved."""
        intent_id = "test-client-render-456"

        await test_db_store.record_dispatch_timings(intent_id, router_ms=150)
        await test_db_store.record_dispatch_timings(intent_id, first_render_ms=90)
    ...
    @pytest.mark.asyncio
    async def test_client_timings_both_stt_and_render(self, test_db_store):
        """Both stt_ms and first_render_ms can be reported for the same intent."""
        intent_id = "test-client-both-789"

        await test_db_store.record_dispatch_timings(intent_id, router_ms=120)
        await test_db_store.record_dispatch_timings(
            intent_id,
            stt_ms=450,
            first_render_ms=85,
    ...
    @pytest.mark.asyncio
    async def test_client_timings_creates_row_if_not_exists(self, test_db_store):
        """Client timing report creates dispatch_timings row if intent_id doesn't exist yet."""
        intent_id = "test-client-create-row-999"

        # No prior row exists
        timings_before = await test_db_store.get_dispatch_timings(intent_id)
        assert timings_before is None

        # Report client timing only (no server-side fields yet)
        await test_db_store.record_dispatch_timings(intent_id, stt_ms=200)

        # Row should now exist
        timings_after = await test_db_store.get_dispatch_timings(intent_id)
...
class TestE2EAggregationHelpers:
    ...
    @pytest.mark.asyncio
    async def test_store_latency_percentiles_method(self, test_db_store):
        ...
        percentiles = await test_db_store.get_latency_percentiles()
    ...
    @pytest.mark.asyncio
    async def test_latency_percentiles_returns_empty_when_no_data(self, test_db_store):
        """get_latency_percentiles() returns {} when no timings exist."""
        percentiles = await test_db_store.get_latency_percentiles()
        assert percentiles == {}

    @pytest.mark.asyncio
    async def test_latency_percentiles_with_since_filter(self, test_db_store):
        ...
        await test_db_store.record_dispatch_timings(old_intent, router_ms=1000)
        ...
        await test_db_store.record_dispatch_timings(recent_intent, router_ms=100)
        ...
        recent_percentiles = await test_db_store.get_latency_percentiles(since=five_mins_ago)
...
class TestE2ECompleteDispatchFlow:
    ...
    @pytest.mark.asyncio
    async def test_complete_hot_path_dispatch_has_all_stages(self, test_db_store):
    ...
    @pytest.mark.asyncio
    async def test_task_profile_dispatch_has_escalate_ms(self, test_db_store):
...
class TestE2EExistingTestsPass:
    ...
    @pytest.mark.asyncio
    async def test_store_initialization_with_dispatch_timings(self, test_db_store):
    ...
    @pytest.mark.asyncio
    async def test_record_dispatch_timings_idempotent(self, test_db_store):
        ...
        await test_db_store.record_dispatch_timings(intent_id, fetch_total_ms=200)
        await test_db_store.record_dispatch_timings(intent_id, synthesize_total_ms=300)
        await test_db_store.record_dispatch_timings(intent_id, sse_emit_ms=50)
    ...
    @pytest.mark.asyncio
    async def test_get_latency_percentiles_handles_sparse_data(self, test_db_store):
        """get_latency_percentiles() handles rows where only some stages are set."""
        # Create rows with different stage combinations
        await test_db_store.record_dispatch_timings("id1", router_ms=100)
        await test_db_store.record_dispatch_timings("id2", router_ms=150, fetch_total_ms=200)
        await test_db_store.record_dispatch_timings("id3", synthesize_total_ms=300)
        await test_db_store.record_dispatch_timings("id4", router_ms=200, synthesize_total_ms=400)
