# Fetch Strand: Lookup Intent - Logs Kind

This document defines the fetch strategy for `intent_type: lookup` with `lookup_kind: logs`.

## What We Fetch

For a logs lookup query, we need recent log output and related diagnostic data:

1. **Logs**: Recent log output from pods/services
2. **Events**: Kubernetes events for the namespace
3. **Pod status**: Current state of pods (for context)
...
For logs lookup queries, include these context fields if available:
