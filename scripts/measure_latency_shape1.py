#!/usr/bin/env python3
"""
Shape 1 Latency Baseline Measurement

Runs the step1_multi_status utterance (Shape 1) 30+ times to collect timing data.
This is the first demo-step shape: multi-intent status query (pbx-web + whisper-stt).
"""
...
# Shape 1 utterance: multi-intent status query
SHAPE_1_UTTERANCE = {
...
async def main():
    """Main execution."""
    print("="*60)
    print("Shape 1 Latency Baseline Measurement")
    print("="*60)
    ...
    # Run Shape 1 utterance
    results = await run_utterance_batch(SHAPE_1_UTTERANCE, RUNS_PER_UTTERANCE)
    ...
    # Print results
    print(f"\n{'='*60}")
    print("LATENCY ANALYSIS RESULTS - SHAPE 1")
    print(f"{'='*60}")
    ...
    results_file = Path(f"data/latency-baseline-shape1-{timestamp}.json")
    results_file.parent.mkdir(parents=True, exist_ok=True)

    output = {
        "metadata": {
            "shape": "step1_multi_status",
            "shape_description": "Multi-intent status query (pbx-web + whisper-stt)",
            "utterance": SHAPE_1_UTTERANCE["utterance"],
            "timestamp": datetime.now().isoformat(),
