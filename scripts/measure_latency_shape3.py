#!/usr/bin/env python3
"""
Shape 3 Latency Baseline Measurement

Runs the step3_brainstorm utterance (Shape 3) 30+ times to collect timing data.
This is the third demo-step shape: brainstorm (pbx-web).
"""
...
async def main():
    """Main execution."""
    print("="*60)
    print("Shape 3 Latency Baseline Measurement")
    print("="*60)
    ...
    # Run Shape 3 utterance
    results = await run_utterance_batch(SHAPE_3_UTTERANCE, RUNS_PER_UTTERANCE)
    ...
    # Print results
    print(f"\n{'='*60}")
    print("LATENCY ANALYSIS RESULTS - SHAPE 3")
    print(f"{'='*60}")
    ...
    results_file = Path(f"data/latency-baseline-shape3-{timestamp}.json")
    results_file.parent.mkdir(parents=True, exist_ok=True)

    output = {
        "metadata": {
            "shape": "step3_brainstorm",
            "shape_description": "Brainstorm (pbx-web)",
            "utterance": SHAPE_3_UTTERANCE["utterance"],
            "timestamp": datetime.now().isoformat(),
